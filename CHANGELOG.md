...
* feat: surface company_id ticket scoping for multi-company portals by @jeffersongoncalves in https://github.com/jeffersongoncalves/filament-help-desk/pull/102

**Full Changelog**: https://github.com/jeffersongoncalves/filament-help-desk/compare/1.7.0...1.8.0

## 1.7.0 - 2026-09-18

**Added:** "Test Connection" action on the EmailChannel form (Admin panel) — validates IMAP/webhook driver credentials before saving, via `laravel-help-desk` v1.11's `EmailDriver::testConnection()`. See #94.

**Docs:** Version Compatibility table now links each branch and reports the correct `laravel-help-desk` constraint (`^1.11`). See #95.
...
The User panel runs on a satellite application now. Requires `jeffersongoncalves/laravel-help-desk` `^1.9`. Filament v3 — one implementation difference from 3.4.0 and 2.4.0, noted below.
...
```
- **The list** is fed by `forActor()`, and paged, filtered, searched and sorted by the central application. Filament v3 builds a table from an Eloquent query and offers no other data source, so on this branch the rows come from a `ListTickets::getTableRecords()` override rather than `Table::records()`. It reads the page, the page size, the search term, the sort and the active filters the same way Filament would, and the query is never built. Not by narrowing the page the satellite happens to hold — a list that filters 25 rows out of 300 looks filtered and is wrong, with nothing on screen saying so.
